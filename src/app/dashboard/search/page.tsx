  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-4xl mx-auto">
      ...
      <div className="text-center mb-10">
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-brand-500/10 text-brand-300 text-sm font-medium mb-6">
      ...
      <form onSubmit={handleSearch} className="relative group">
        ...
        <div className="relative flex items-center bg-surface-card border border-surface-border rounded-2xl overflow-hidden p-2">
          ...
          <input
            ...
            className="flex-1 bg-transparent border-none text-white px-4 py-3 focus:outline-none placeholder:text-slate-500 text-lg"
          />
          <button
            type="submit"
            disabled={loading || !query.trim()}
            className="px-6 py-3 text-sm font-semibold text-white gradient-brand rounded-xl hover:opacity-90 transition-all disabled:opacity-50 cursor-pointer flex items-center gap-2"
          >
      ...
      {searched && !loading && (
        <div className="space-y-4 mt-10">
          <div className="flex items-center justify-between text-sm text-slate-400 pb-2 border-b border-surface-border/50">
          ...
          {results.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {results.map((product) => (
                <div key={product.id} className="glass rounded-2xl p-5 flex gap-4 hover:border-brand-500/30 transition-all group">
                  {product.image_url ? (
                    <img src={product.image_url} alt={product.name} className="w-20 h-20 rounded-xl object-cover shrink-0" />
                  ) : (
                    <div className="w-20 h-20 rounded-xl bg-surface-dark flex items-center justify-center shrink-0">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-400 line-clamp-2 mb-2">{product.description || 'No description available.'}</p>
            ...
            <div className="text-center py-12 glass rounded-2xl">
              ...
              <p className="text-lg font-semibold text-slate-400">No matching products found</p>
